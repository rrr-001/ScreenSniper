...
bool embedRobustWatermark(cv::Mat &image, const std::string &textInput) {
    ...
    cv::Mat imgBGR;
    if (work.channels() == 3) imgBGR = work;
    else if (work.channels() == 1) cv::cvtColor(work, imgBGR, cv::COLOR_GRAY2BGR);
    else return false;
    ...
    for (char c : fixed) payloadBytes.push_back(static_cast<unsigned char>(c));
    // add checksum byte
...
bool extractRobustWatermark(const cv::Mat &image, std::string &outText) {
    ...
    for (int i = 0; i < expectedBits * REPEAT; ++i) chosenBlocks.push_back(distBlock(rng));
    ...
    for (int i = 0; i < expectedBits; ++i) recoveredBits[i] = (votes[i] > 0) ? 1 : 0;

    // convert to bytes
    std::vector<unsigned char> recBytes = bitsToBytes(recoveredBits);

    // verify sync exists at start
    if ((int)recBytes.size() < expectedPayloadBytes) return false;
    bool syncOk = true;
    for (int i = 0; i < SYNC_LEN; ++i) {
        if (recBytes[i] != SYNC_PATTERN[i]) { syncOk = false; break; }
    }
    if (!syncOk) {
    ...
    for (unsigned char c : wmBytes) outText.push_back(static_cast<char>(c));
    return true;
}
